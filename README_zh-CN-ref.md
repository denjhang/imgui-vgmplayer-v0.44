...
* [8. 故障排查与更新日志](#8)
  ...
  * [8.8. SSG 到 OPM 转换修复与音量微调 (v0.927)](#8-8)
...
#### 4.2.5. SSG 到 OPM 的详细转换规则
<a id="4-2-5"></a>
为了在 YM2151 (OPM) 上模拟 YM2203 和 YM2608 的 SSG 部分，播放器采用了一套详细的转换规则，将 SSG 的方波和白噪音映射到 OPM 的 FM 通道上。

##### 通道映射

- **YM2203**: YM2203 拥有 3 个 FM 通道和 3 个 SSG 通道。转换时，FM 通道直接映射到 OPM 的 0-2 通道。SSG 的 3 个方波通道则直接映射到 OPM 的 3-5 通道。此外，SSG 的白噪音被路由到一个专用的 OPM 通道（默认为通道 7），该通道被配置为噪音模式。这种直接映射的方式保证了所有声道都能同时发声。
- **YM2608**: YM2608 拥有 6 个 FM 通道和 3 个 SSG 通道，总共 9 个潜在的 FM/SSG 声源，而 OPM 只有 8 个通道。为了解决这个问题，播放器在转换前会进行一次预扫描（`vgm_analyze_and_set_ssg_mapping`），分析乐曲中 SSG 通道的使用情况，并将其动态地映射到未被 FM6 使用的 OPM 通道上。这是一种基于优先级和可用性的动态分配策略。

##### Key On / Key Off 逻辑

SSG 的发声机制与 FM 不同，它没有独立的 Key On/Off 寄存器。一个音符的触发（Key On）和停止（Key Off）通常由两种方式控制：

1.  **Mixer 寄存器 (`0x07`)**: 通过控制每个通道的 `TONE` 位（bit 0-2）的开关来直接控制发声。
2.  **音量寄存器 (`0x08`-`0x0A`)**: 在 Mixer 中 `TONE` 位保持开启的情况下，通过将音量从 0 调到非 0 值来触发 Key On，反之则为 Key Off。

当前版本的转换逻辑同时兼顾了这两种情况，确保了无论是通过 Mixer 还是音量控制的 SSG 音符都能被正确地转换为 OPM 的 Key On/Off 事件。

##### 音量转换

SSG 的音量是 4-bit（0-15），而 OPM 使用 7-bit 的总电平（Total Level, TL）来控制音量，其中 0 是最大声，127 是静音。转换时，播放器使用一个查找表（`VOL_TO_TL`）将 SSG 音量映射到合适的 TL 值。

为了优化听感，当前版本对方波和白噪音应用了不同的音量调整：
- **方波**: 在查找表的基础上增加了 3 个单位的衰减值，使其音量约为原始的 0.9 倍，声音更柔和。
- **白噪音 (仅 YM2203)**: 在查找表的基础上减少了 13 个单位的衰减值，使其音量约为原始的 1.8 倍，在混合音效中更清晰可闻。
...
### 8.8. SSG 到 OPM 转换修复与音量微调 (v0.927)
<a id="8-8"></a>
**v0.927 (2025-10-17)**
- **核心修复**: 彻底重构了 YM2203 SSG 到 OPM 的转换逻辑。之前版本中，SSG 声道的 Key-On 事件仅在 Mixer 寄存器（`0x07`）写入时触发，导致许多依赖音量寄存器（`0x08`-`0x0A`）来控制音符开关的乐曲出现声道静音问题。新逻辑通过在写入音量寄存器时，检测音量从 0 到非 0 的变化来正确触发 Key-On，并在音量变为 0 时触发 Key-Off，从而解决了此问题。
- **逻辑隔离**: 严格区分了 YM2203 和 YM2608 的转换路径，确保 YM2203 的直接映射和 YM2608 的动态映射互不干扰，提高了代码的清晰度和稳定性。
- **音量微调**:
    - 将 SSG 方波音量调整为约 0.9 倍（衰减值增加 3），使其听感更柔和。
    - 将 SSG 白噪音音量调整为约 1.8 倍（衰减值减少 13），使其在混合音效中更清晰可闻。
